...
const ChatBubbleWithReply = props => {
  ...
  const reply_header =
    position == 'right'
      ? `you replied to ${currentMessage.reply_to}`
      : `${currentMessage.user.name} replied to ${currentMessage.reply_to}`
    ...
    <View style={styles[`${position}_container`]}>
      <View style={styles[`${position}_wrapper`]}>
        <View style={styles.reply_to_container}>
          <Text style={[styles.reply_to, {color: reply_to_color}]}>
            {reply_header}:
          </Text>
          <View style={styles.reply_to_msg_container}>
            <Text style={[styles.reply_to_msg, {color: reply_to_msg_color}]}>
              {currentMessage.reply_to_msg}
            </Text>
          </View>
        </View>
        <MessageText {...props} />
        {currentMessage.image && <MessageImage {...props} />}
        {children}
        <Time {...props} />
      </View>
    </View>
